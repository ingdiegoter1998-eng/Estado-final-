...
new_block = """            {% comment %} BOTONES DE SEGUIMIENTO {% endcomment %}
            <div class="card shadow-sm mb-4">
                <div class="card-header bg-white border-bottom-0"><h6 class="mb-0 fw-bold">Información de Seguimiento</h6></div>
                <div class="card-body action-stack text-center pt-2">
                    ...
                    <button type="button" class="btn btn-outline-info mb-2 w-100 fw-medium d-flex align-items-center justify-content-center gap-2" data-bs-toggle="modal" data-bs-target="#modalEstadoLectura">
                        <i class="bx bx-check-double fs-5"></i> Estado de lectura
                    </button>
                    {% if accesos_oficinas %}
                    <button type="button" class="btn btn-outline-secondary w-100 fw-medium d-flex align-items-center justify-content-center gap-2" data-bs-toggle="modal" data-bs-target="#modalAccesosOficinas">
                    ...
                    <div class="timeline-item mb-4 position-relative">
                        ...
                        <div class="timeline-content card border-0 shadow-sm p-3" style="border-radius: 12px;">
                            ...
                            <div class="timeline-body mt-2">
                                ...
                                <p class="mb-0 text-secondary small bg-light p-2 rounded" style="white-space: pre-line;">{{ evento.descripcion }}</p>
                                {% endif %}
                            </div>
                        </div>
                    </div>
...
# Reconstruct lines
# We want to keep everything before 381 (0-indexed 380)
# We add `new_block`
# We skip everything from 381 to 467 (0-indexed 466)
# Keep 468 to end
# AND replace `<!-- ===================== MODALES ===================== -->` with `modales_html + "\n<!-- ===================== MODALES ===================== -->"`

start_idx = 381 # line 382
end_idx = 469 # inclusive up to line 470 `{% endif %}`

final_lines = lines[:start_idx] + [new_block] + lines[end_idx:]

# Join and replace modales section
final_text = "".join(final_lines)
final_text = final_text.replace("<!-- ===================== MODALES ===================== -->", modales_html + "<!-- ===================== MODALES ===================== -->")

with open("correspondencia/templates/correspondencia/usuario/detalle_correspondencia.html", "w", encoding="utf-8") as f:
    f.write(final_text)
